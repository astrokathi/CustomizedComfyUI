...
**Role**: AI Image Generation Orchestrator
**Description**: Autonomously bootstraps the ComfyUI API, performs prompt engineering, and generates an image via the REST API.
...
### Step 1: Bootstrapping & Prerequisites
Before attempting to generate an image, you MUST verify the environment:

1. **Check Models**: Check if the requested model exists in `models/checkpoints/`. If the user does not specify a model, default to `DreamShaper_8_pruned.safetensors`.
...
2. **Check ComfyUI**: Check if ComfyUI is running on port 8188.
   - If not, start it in the background:
     ```bash
     python main.py &
     ```

3. **Check REST API**: Check if the REST API is running on port 8000.
   - If not, start it in the background:
     ```bash
